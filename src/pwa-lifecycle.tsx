...
const commitNotes=async()=>{const response=await fetch(`https://api.github.com/repos/${GITHUB_REPO}/commits?per_page=1`,{cache:"no-store"});if(!response.ok)throw new Error("Commit unavailable");const data=await response.json();return parseNotes(data[0]?.commit?.message||"")};
...
export default function PwaLifecycle(){
 ...
 useEffect(()=>{
  let active=true;
  const check=async(initial=false)=>{const previous=localStorage.getItem(LAST_VERSION);if(initial&&previous===null)localStorage.setItem(LAST_VERSION,__APP_VERSION__);try{const latest=await deployedVersion();if(!active)return;if(newerThan(latest,__APP_VERSION__)){setNextVersion(cleanVersion(latest));setUpdate(true)}}catch(error){if(!warned.current){warned.current=true;console.warn("BLENDIN update check unavailable.",error)}}if(initial&&previous!==null&&previous!==__APP_VERSION__){try{const found=await commitNotes();if(active)setNotes(found)}catch{}finally{if(active)localStorage.setItem(LAST_VERSION,__APP_VERSION__)}}};
  check(true);
  const refresh=()=>{if(document.visibilityState!=="hidden"){void check(false);void registration.current?.update().catch(()=>{})}};
  const timer=setInterval(refresh,30000);
  addEventListener("focus",refresh);addEventListener("online",refresh);document.addEventListener("visibilitychange",refresh);
  return()=>{active=false;clearInterval(timer);removeEventListener("focus",refresh);removeEventListener("online",refresh);document.removeEventListener("visibilitychange",refresh)};
 },[]);
 ...
 useEffect(()=>{let active=true;setNextNotes(null);if(update)commitNotes().then(value=>{if(active)setNextNotes(value)}).catch(()=>{});return()=>{active=false}},[update,nextVersion]);
 ...
 if(update)return <Modal blocking icon={logo} over="UPDATE AVAILABLE" title="Please update to continue"><p>Version {nextVersion||"being checked…"}</p>{nextNotes&&<><h3 className="patch-heading">What's New?</h3><NotesView notes={nextNotes}/></>}{updateError&&<p role="status">{updateError}</p>}<button className="primary wide" disabled={updating} onClick={updateNow}>{updating?"Updating…":"Update Now"}</button></Modal>;
 if(notes)return <Modal icon={logo} over="LATEST CHANGES" title="What's New?" close={()=>setNotes(null)}><p className="patch-title">{notes.title}</p><p>Version {__APP_VERSION__}</p><NotesView notes={{...notes,title:""}}/><button className="primary wide" onClick={()=>setNotes(null)}>Continue</button></Modal>;
