...
import {BookOpen,Briefcase,Camera,Car,Castle,ChevronLeft,CloudSun,Code2,Coffee,Eye,Film,Gamepad2,Globe2,GraduationCap,GripVertical,House,Leaf,MapPin,Moon,Music2,PawPrint,Play,RefreshCw,Rocket,Settings,Shapes,Shirt,Shuffle,Smartphone,Star,Sun,TreePine,Trophy,Users,WandSparkles,type LucideIcon} from "lucide-react";
import {forLanguage} from "./word-data";
import {getSmartHintExplanation} from "./hint-explanations";
declare const __APP_VERSION__:string;
type Screen="home"|"setup"|"topics"|"reveal"|"firstPlayer"|"imposterReveal"|"clue"|"discussion"|"voting"|"guess"|"result"|"scores"|"settings"|"changelog"|"help";
const APP_SCREENS=new Set<Screen>(["home","setup","topics","reveal","firstPlayer","imposterReveal","clue","discussion","voting","guess","result","scores","settings","changelog","help"]);
type Player={id:string,name:string,points:number,wins:number,correct:number,rounds:number};
const cardSkins=[{id:"classic",name:"Classic"},{id:"grid",name:"Midnight Grid"},{id:"gilded",name:"Gilded"}] as const;
type CardSkin=typeof cardSkins[number]["id"];
const changelog=[{"version":"1.0.5","title":"Make it your table","notes":["Catch up on past updates in Changelog from the home menu.","Choose Classic, Midnight Grid or Gilded cards in Settings, or let the game pick a design each round.","Impostor and Jester names now appear in equally sized panels so both are easy to read.","Open Settings from the home menu. The extra gear button has been removed."]},{"version":"1.0.4","title":"A clearer role reveal","notes":["With voting off, tap Reveal Roles to see the Impostor and Jester without announcing a winner or loser.","The Jester’s secret word now has its own box, with a smaller, italic goal above it.","Removed the extra “Keep holding to view” text from private cards."]},{"version":"1.0.0–1.0.3","title":"Earlier updates","notes":["The Jester’s role name is now as large and easy to read as the Impostor’s.","Play with the Jester even when voting is off. Reveal the roles casually without announcing who won.","The Jester has a purple card and a clear goal: get more votes than anyone else.","Removed Known Topic and Quick Round from the game mode choices.","Added the optional Jester role for players who want to look suspicious and attract votes.","Update messages are easier to read and scroll through. The game restarts after an update finishes.","See what’s new before updating, with a clear version number and a list of changes.","Added short word meanings to supported non-Impostor cards to help players understand unfamiliar words.","Fixed categories showing unrelated words and the duplicate sun icon in Settings. Removed Statistics.","Added an option to install BlendIn from your browser and a reminder when an update is ready.","Drag players up or down the list to match your seating order. The handle stays highlighted while you move them.","Skip someone who has not viewed their card yet and move them to the end of the line.","Accidentally showed the wrong card? Change the secret word and start card viewing again from the first player.","Impostor hints are now just 1–2 words, with short explanations after the round. Harder settings give less obvious clues.","Play offline after the game has fully downloaded during an online visit.","Improved phone navigation so going back returns to the previous screen, with fewer accidental refreshes.","Category names on private cards now follow the selected difficulty and game mode.","The game is now called BlendIn: Who’s the Imposter?","Results show the secret word, votes, outcome, Impostor and hint in that order.","After voting, go straight to the results unless a caught Impostor has a final guess to make.","Caught Impostors can guess the secret word or skip and lose before the answer is revealed.","Impostor Escape now works with voting to decide who gets a final guess."]}];
const names=["Alex","Sam","Jordan","Taylor"];
const ico=(n:string)=>{
 const icons:Record<string,LucideIcon>={gear:Settings,sun:Sun,moon:Moon,play:Play,back:ChevronLeft,eye:Eye,shuffle:Shuffle};
   ...
   <section>
    {!publicReveal ? (
     <>
      ...
      <h1>Ready to reveal<br/>{voteMode==="off"?"the roles?":"the Impostor?"}</h1>
      <small>Make sure everyone can see the screen.</small>
      <button className="primary wide red" onClick={revealRound}>{voteMode==="off"?"Reveal Roles":"Reveal the Impostor"} →</button>
     </>
    ) : (
     <>
      ...
      {voteMode!=="off"&&<div className={"round-verdict "+(winner==="players"?"players-win":"impostor-win")}><small>{winner==="jester"?"JESTER VOTED OUT":caught?"IMPOSTOR CAUGHT":"GROUP VOTED INCORRECTLY"}</small><b>{winner==="jester"?"JESTER WINS":winner==="players"?"PLAYERS WIN":escapedRound?"THE IMPOSTOR ESCAPED":caught&&escape&&!roundResolved?"FINAL GUESS AVAILABLE":"IMPOSTORS WIN"}</b>{caught&&<span>{caughtImps.map(id=>players.find(p=>p.id===id)?.name).join(" & ")}{caughtImps.length>1?" were":" was"} among the highest-voted players.</span>}</div>}
      <div className="role-identities"><div className="role-identity role-impostor"><small>IMPOSTOR{imps.length>1?"S":""}</small><h2>{imps.map(id=>players.find(p=>p.id===id)?.name).join(" & ")}</h2></div>{jesterId&&<div className="role-identity role-jester"><small>JESTER</small><h2>{players.find(p=>p.id===jesterId)?.name}</h2>{voteMode!=="off"&&<span>{winner==="jester"?"Won as the sole highest-voted player.":"Did not win this round."}</span>}</div>}</div>
